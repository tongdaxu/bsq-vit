...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="Evaluate public tokenizer", add_help=False
    )
    parser.add_argument(
        "--model",
        default="dalle-dvae",
    ...
    parser.add_argument(
        "--imagenet-val-path",
        default="/storage/Datasets/ILSVRC2012/val/",
        type=str,
        help="path to imagenet-1k val",
    )
    parser.add_argument(
        "--coco-val-path",
        default="/storage/Datasets/ILSVRC2012/coco/",
        type=str,
        help="path to coco val",
    )
    parser.add_argument(
        "--interpolation",
        default=None,
    ...
    parser.add_argument(
        "--save-to-folder", default=None, type=str, help="save results to folder"
    )
...
def main(args):
    ...
    if args.model == "dalle-dvae":
        ...
        if args.model == "vqgan1024":
            model = VQGAN(
                "third_party/vqgan/vq-f8-n256/config.yaml",
                "third_party/vqgan/vq-f8-n256/model.ckpt",
            )
        elif args.model == "vqgan16384":
            model = VQGAN(
                "third_party/vqgan/vq-f8/config.yaml",
                "third_party/vqgan/vq-f8/model.ckpt",
            )
        elif args.model == "vqgan-kl":
            model = VQGAN(
                "third_party/vqgan/vqgan_gumbel_f8/configs/model.yaml",
                "third_party/vqgan/vqgan_gumbel_f8/checkpoints/last.ckpt",
                is_gumbel=True,
            )
    ...
    imagenet_dataset = SimpleDataset(args.imagenet_val_path, transforms=val_transform)
    if args.distributed:
        imagenet_sampler = torch.utils.data.distributed.DistributedSampler(
            imagenet_dataset, shuffle=False
        )
    else:
...
def evaluate(
    ...
    full_eval=False,
    num_samples=1000,
    groundtruth_npz="imagenet_val_256x256.npz",
):
    from transcoder.evaluations.fid.inception import InceptionV3

    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[2048]
    inception_v3 = InceptionV3([block_idx], normalize_input=not args.zero_mean).cuda(
        args.gpu
    )
    ...
    with torch.inference_mode():
        for iter, (_, data) in enumerate(data_loader):
            ...
            if full_eval:
                ...
                pred_ssim, pred_msssim = get_ssim_and_msssim(
                    x, x_recon, zero_mean=args.zero_mean
                )
                gathered_ssim = [
                ...
                pred_lpips = get_lpips(
                    x, x_recon, zero_mean=args.zero_mean, network_type="alex"
                )
                gathered_lpips = [
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate public tokenizer", parents=[get_args_parser()]
    )

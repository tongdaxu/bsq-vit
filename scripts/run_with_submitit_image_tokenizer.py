...
import submitit
import scripts.main_image_tokenizer as main_image_tokenizer


def parse_args():
    parser = main_image_tokenizer.get_args_parser()
    parser = argparse.ArgumentParser("Submitit for VQ-GAN training", parents=[parser])
    parser.add_argument(
    ...
    parser.add_argument(
        "--timeout", default=10080, type=int, help="Duration of the job"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--partition", default="rtxa", type=str, help="Partition where to submit"
    )
...
def get_shared_folder() -> Path:
    user = os.getenv("USER")
    if Path("/u/").is_dir():
        p = Path(f"/u/{user}/experiments/image_tokenizer")
...
class Trainer(object):
    ...
    def __call__(self):
        ...
        main_image_tokenizer.main(self.args)
...
def main():
    ...
    executor.update_parameters(name="image_tokenizer")

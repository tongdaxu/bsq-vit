...
from transcoder.utils.misc import (
    check_loss_nan,
    get_metrics_dict,
    get_grad_norm,
    tensor2image,
...
import wandb
from irec import GaussianPFR, GaussianPTQ
...
def get_args_parser():
    ...
    parser.add_argument(
        "--eval-freq", default=20_000, type=int, help="evaluation frequency"
    )
    parser.add_argument("--save-freq", default=1_000, type=int, help="save frequency")

    # EMA
    parser.add_argument(
        "--use-ema", action="store_true", help="use exponential moving average"
    )
    ...
    parser.add_argument(
        "--ema-eval-freq", default=40_000, type=int, help="evaluation frequency for EMA"
    )
    ...
    parser.add_argument(
        "--vis-freq", default=0, type=int, help="visualization frequency"
    )
    parser.add_argument(
        "-j",
        "--workers",
    ...
    parser.add_argument(
        "--skip-quantize", action="store_true", help="skip quantize at evaluation"
    )
    parser.add_argument(
        "--world-size",
        default=1,
...
def main(args):
    ...
    assert not config.data.zero_mean or not config.model.params.get(
        "logit_laplace", False
    ), "logit laplace mode is only compatible with the input being [0, 1]"
    model.cuda(args.gpu)
    ...
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(
            ...
            device_ids=[args.gpu],
            bucket_cap_mb=200,
            find_unused_parameters=config.loss.params.disc_start
            > 0,  # only used if the discriminator kicks in later
        )

    if args.use_ema:
        ema_model = ModelEmaV2(
            model, args.ema_decay, device=torch.cpu() if args.cpu_ema else None
        )
    ...
    config.optimizer.params.lr = (
        config.optimizer.base_lr * args.world_size * config.data.batch_size
    )
    print(f"base LR is {config.optimizer.params.lr}")

    optimizer_generator = config_utils.instantiate_optimizer_from_config(
        config.optimizer,
        get_params_for_weight_decay(
            model,
            zero_wd_keys=config.optimizer.get("zero_wd_keys", []),
    ...
    enable_scaler = not config.optimizer.disable_amp and (
        not config.optimizer.get("use_bf16", False)
        or config.optimizer.get("force_bf16_scaler", False)
    )
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if args.use_ema:
                if (
                    ...
                    print(
                        "EMA speicifed but not found in the checkpoint, will reset EMA to the model state."
                    )
                    ema_model.set(model)
            print(result)
            print(checkpoint["iter"])
            if args.start_iter >= config.loss.params.disc_start:
                result = criterion.discriminator.load_state_dict(
                    checkpoint["state_dict_discriminator"], strict=True
                )
        ...
        if os.path.isfile(latest):
            ...
            if args.use_ema:
                if (
                    "ema_state_dict" in latest_checkpoint
                    and latest_checkpoint["ema_state_dict"] is not None
                ):
            ...
            if args.start_iter >= config.loss.params.disc_start:
                result = criterion.discriminator.load_state_dict(
                    latest_checkpoint["state_dict_discriminator"], strict=True
                )
                optimizer_discriminator.load_state_dict(
                    latest_checkpoint["optimizer_discriminator"]
                )
                print(result)

            optimizer_generator.load_state_dict(
                latest_checkpoint["optimizer_generator"]
            )
            scaler.load_state_dict(latest_checkpoint["scaler"])
            print(
                "=> loaded latest checkpoint '{}' (iter {})".format(
                    latest, latest_checkpoint["iter"]
                )
            )
    ...
    train_dataset = config_utils.instantiate_dataset_from_config(
        config.data.train, train_transform
    )
    valid_dataset = config_utils.instantiate_dataset_from_config(
        config.data.val, val_transform
    )
    if args.distributed:
    ...
    lr_scheduler = (
        ...
        if config.optimizer.get("lr_scheduler_config", None) is not None
        else None
    )
    ...
    h_max_meter = AverageMeter("H-max", ":6.2f")
    h_min_meter = AverageMeter("H-min", ":6.2f")
    ...
    metric_names = [
        ...
        "train/logits_fake",
        "train/disc_r1_loss",
        "train/disc_r1_loss_scale",
    ]
    if config.loss.get("logit_laplace_weight", 0) > 0:
        assert dist_utils.get_model(model).logit_laplace, (
            "The model has to be compatible with logit laplace loss"
        )
    ...
    while True:
        ...
        for data, _ in train_loader:
            ...
            with amp.autocast(
                enabled=not config.optimizer.disable_amp,
                dtype=torch.bfloat16 if config.optimizer.use_bf16 else torch.float16,
            ):
                x_recon, qloss, qinfo = model(x)

            skip_disc = iter < config.loss.params.disc_start and config.loss.params.get(
                "skip_disc", False
            )
            ...
            for optimizer_idx in range(2 - skip_disc):
                ...
                with amp.autocast(
                    ...
                    loss, log = criterion(
                        ...
                        optimizer_idx,
                        iter,
                        dist_utils.get_model(model).get_last_layer(),
                        None,
                        "train",
                    )
                    if "logit_laplace_loss" in qinfo and optimizer_idx == 0:
                        loss = (
                            loss
                            + qinfo["logit_laplace_loss"]
                            * config.loss.logit_laplace_weight
                        )
                ...
                if iter >= config.loss.params.disc_start or not skip_disc:
                    ...
                    scaler.step(
                        optimizer_generator
                    )  # before the discriminator kicks, we must only update the generator
                    if args.use_ema:
                        ema_model.update(model)
                scaler.update()
                ...
                if optimizer_idx == 1 - skip_disc:
                    ...
                    sps_meter.update(
                        time.time() - end, config.data.batch_size * args.world_size
                    )
                    h_meter.update(qinfo["H"])
                    h_min_meter.update(qinfo["H-min"])
                    h_max_meter.update(qinfo["H-max"])
            ...
            if with_wandb:
                ...
                info.update(
                    {
                        "state/H": h_meter.val,
                        "state/H-min": h_min_meter.val,
                        "state/H-max": h_max_meter.val,
                ...
                if args.vis_freq > 0 and iter % args.vis_freq == 0:
                    ...
                    image = tensor2image(
                        rearrange(
                            x[: num_rows**2].detach(),
                            "(r c) ch h w -> (r h) (c w) ch",
                            r=num_rows,
                        ),
                        zero_mean=config.data.zero_mean,
                    )
                    recon = tensor2image(
                        rearrange(
                            x_recon[: num_rows**2].detach(),
                            "(r c) ch h w -> (r h) (c w) ch",
                            r=num_rows,
                        ),
                        zero_mean=config.data.zero_mean,
                ...
                dist_utils.save_on_master(
                    {
                        ...
                        if dist_utils.is_main_process()
                        and iter >= config.loss.params.disc_start
                        else None,
                        "state_dict_discriminator": criterion.discriminator.state_dict()
                        if iter >= config.loss.params.disc_start
                        else None,
...
def evaluate(data_loader, model, args, config, full_eval=False):
    ...
    inception_v3 = InceptionV3(
        [block_idx], normalize_input=not config.data.zero_mean
    ).cuda(args.gpu)
    ...
    pfa_coder = GaussianPFR(int(2**KLD), DIM, SEED).cuda()
    ...
    if full_eval:
        ...
        if args.mode == "kl":
            all_kl_mean = [[] for _ in range(args.world_size)]
            all_kl_min = [[] for _ in range(args.world_size)]
            all_kl_max = [[] for _ in range(args.world_size)]
        ...
        for iter, (data, _) in enumerate(data_loader):
            ...
            with amp.autocast(
                ...
                if args.mode == "train":
                    ...
                    kl_mean = info["H"][None]
                    gathered_kl_mean = [
                        torch.zeros_like(kl_mean) for _ in range(args.world_size)
                    ]
                    torch.distributed.all_gather(gathered_kl_mean, kl_mean)
                    for j in range(args.world_size):
                        all_kl_mean[j].append(gathered_kl_mean[j].detach().cpu())
                    kl_max = info["H-max"][None]
                    gathered_kl_max = [
                        torch.zeros_like(kl_max) for _ in range(args.world_size)
                    ]
                    torch.distributed.all_gather(gathered_kl_max, kl_max)
                    for j in range(args.world_size):
                        all_kl_max[j].append(gathered_kl_max[j].detach().cpu())
                    kl_min = info["H-min"][None]
                    gathered_kl_min = [
                        torch.zeros_like(kl_min) for _ in range(args.world_size)
                    ]
                    torch.distributed.all_gather(gathered_kl_min, kl_min)
                    for j in range(args.world_size):
                        all_kl_min[j].append(gathered_kl_min[j].detach().cpu())
                elif args.mode == "mle" or args.mode == "det" or args.mode == "mrc" or args.mode == "orc":
                    _, _, info = model(x, skip_quantize=args.skip_quantize)

                    mean = info["mean"]
                    std = info["std"]
                    b, c, h, w = mean.shape
                    mean_flat = (
                        mean.reshape(b, DIM, c // DIM, h, w)
                        .permute(0, 2, 3, 4, 1)
                        .reshape(-1, DIM)
                    )
                    std_flat = (
                        std.reshape(b, DIM, c // DIM, h, w)
                        .permute(0, 2, 3, 4, 1)
                        .reshape(-1, DIM)
                    )

                    zhat = torch.zeros_like(mean_flat)
                    tokens = torch.zeros_like(mean.reshape(-1))
                    bs = mean_flat.shape[0] // 16
                    assert mean_flat.shape[0] % bs == 0
                    for i in range(0, mean_flat.shape[0], bs):
                        zhat[i : i + bs, :], tokens[i : i + bs] = pfa_coder.encode(
                            mean_flat[i : i + bs, :], std_flat[i : i + bs, :], args.mode
                        )
                    zhat = (
                        zhat.reshape(b, c // DIM, h, w, DIM)
                        .permute(0, 4, 1, 2, 3)
                        .reshape(b, c, h, w)
                    )
                    if model.module.quantize.input_format == "blc":
                        zhat = zhat.reshape(b, c, -1)
                        zhat = rearrange(zhat, "b l c -> b c l")
                    x_recon = model.module.decode(zhat)
                elif args.mode == "mean":
                    _, _, info = model(x, skip_quantize=args.skip_quantize)
                    mean = info["mean"]
                    zhat = mean
            ...
            if full_eval:
                ...
                gathered_psnr = [
                    torch.zeros_like(pred_psnr) for _ in range(args.world_size)
                ]
                ...
                pred_ssim, pred_msssim = get_ssim_and_msssim(
                    x, x_recon, zero_mean=config.data.zero_mean
                )
                gathered_ssim = [
                    torch.zeros_like(pred_ssim) for _ in range(args.world_size)
                ]
                gathered_msssim = [
                    torch.zeros_like(pred_msssim) for _ in range(args.world_size)
                ]
                ...
                pred_lpips = get_lpips(
                    x, x_recon, zero_mean=config.data.zero_mean, network_type="alex"
                )
                gathered_lpips = [
                    torch.zeros_like(pred_lpips) for _ in range(args.world_size)
                ]
            ...
            if not gt_cached:
                ...
                gathered_pred_x = [
                    torch.zeros_like(pred_x) for _ in range(args.world_size)
                ]
            ...
            gathered_pred_xr = [
                torch.zeros_like(pred_xr) for _ in range(args.world_size)
            ]
    ...
    if full_eval:
        if args.mode == "kl":
            for j in range(args.world_size):
                all_kl_mean[j] = torch.cat(all_kl_mean[j], dim=0)
            all_kl_mean = torch.cat(all_kl_mean, dim=0).numpy()
            print(f"kl mean: {np.mean(all_kl_mean):.4f} (±{np.std(all_kl_mean):.4f})")

            for j in range(args.world_size):
                all_kl_min[j] = torch.cat(all_kl_min[j], dim=0)
            all_kl_min = torch.cat(all_kl_min, dim=0).numpy()
            print(f"kl min: {np.mean(all_kl_min):.4f} (±{np.std(all_kl_min):.4f})")

            for j in range(args.world_size):
                all_kl_max[j] = torch.cat(all_kl_max[j], dim=0)
            all_kl_max = torch.cat(all_kl_max, dim=0).numpy()
            print(f"kl max: {np.mean(all_kl_max):.4f} (±{np.std(all_kl_max):.4f})")
        ...
        for j in range(total_num):
            ...
            all_msssim_reorg.append(
                all_msssim[j % args.world_size][j // args.world_size]
            )
    ...
    if not gt_cached:
        ...
        all_pred_x_reorg = []
        for j in range(total_num):
            all_pred_x_reorg.append(
                all_pred_x[j % args.world_size][j // args.world_size]
            )
    ...
    if all_codes is not None:
        ...
        print(
            f"Codebook usage: {all_codes.size(0)} / {2**config.model.params.embed_dim} ({usage_rate * 100:.02f}%)"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Image Tokenizer", parents=[get_args_parser()]
    )

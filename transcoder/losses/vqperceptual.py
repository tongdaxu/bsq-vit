...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(
        ...
        zero_mean=False,
        num_frames=1,
        codebook_rampup_multiplier=1.0,
        codebook_rampup_steps=0,
        **kwargs,
    ):
        """
        Inputs:
    ...
    def forward(
        ...
        if global_step < self.discriminator_iter_start and self.skip_disc_before_start:
            ...
            disc_factor = 1.0 if global_step >= self.discriminator_iter_start else 0.0
        ...
        if optimizer_idx == 0:
            ...
            if self.codebook_rampup_steps > 0:
                rampup_rate = (
                    min(self.codebook_rampup_steps, global_step)
                    / self.codebook_rampup_steps
                )
                cb_weight = self.codebook_weight * (
                    1.0 * rampup_rate
                    + self.codebook_rampup_multiplier * (1 - rampup_rate)
                )
            else:
                cb_weight = self.codebook_weight
            loss = (
                nll_loss + d_weight * disc_factor * g_loss + cb_weight * codebook_loss
            )
        ...
        if optimizer_idx == 1:
            ...
            if self.disc_type == "stylegan3d":
                inputs = rearrange(
                    inputs, "(n t) c h w -> n c t h w", t=self.num_frames
                )
                reconstructions = rearrange(
                    reconstructions, "(n t) c h w -> n c t h w", t=self.num_frames
                )
                ...
                log.update(
                    {
                        ...
                        "{}/disc_r1_loss_scale".format(
                            split
                        ): r1_loss_scale.detach().mean(),
                    }
                )
